...
function renderEnemyRoleGuessCard({ apiRaw, enemyIds, idToName }) {
  ...
  const rows = ids.map((cid) => {
    ...
    const d = detail?.[k] || {};
    const roleGamesMap = d?.role_games ?? d?.roleGames ?? {};

    const total = Number(d?.total_games ?? d?.totalGames ?? 0);
    const assignedGames = Number(roleGamesMap?.[role] ?? 0);
    const assignedShare = total > 0 ? assignedGames / total : null;

    // 참고용(개인 top role 기준) - 필요하면 UI에 작게 표시
    const topRole = String(d?.top_role ?? d?.topRole ?? "UNKNOWN");
    const topGames = Number(d?.top_games ?? d?.topGames ?? 0);
    const topShare = Number(d?.top_share ?? d?.topShare ?? 0);

    return {
      cid,
      name: idToName?.[cid] || "UNKNOWN",
      role, // ✅ 최종 배정 role
      // ✅ 표시용: "배정 role 기준" confidence
      total,
      assignedGames,
      assignedShare,
      // (optional) 개인 top role 참고
      topRole,
      topGames,
      topShare,
    };
  ...
  return (
    <div className="card" style={{ marginTop: 12 }}>
      ...
      <div className="p" style={{ marginTop: 0 }}>
        used enemy_role column: <b>{usedRoleCol ? "YES" : "NO"}</b> — (서버 guess 그대로 표시)
      </div>
      ...
      <div style={{ display: "grid", gap: 8, marginTop: 10 }}>
        {rows.map((r) => (
          <div key={`enemy_guess_${r.cid}`} className="card" style={{ background: "rgba(255,255,255,0.02)" }}>
            ...
            {/* ✅ 핵심 수정: 배정 role 기준 share + role/total 표시 */}
            <div className="p" style={{ marginTop: 6 }}>
              confidence(share): <b>{formatPct01(r.assignedShare)}</b>{" "}
              <span style={{ opacity: 0.9 }}>
                — role/total games: {Number.isFinite(Number(r.assignedGames)) ? r.assignedGames : "(n/a)"} /{" "}
                {Number.isFinite(Number(r.total)) ? r.total : "(n/a)"}
              </span>
            </div>

            {/* 참고: 배정 role != 개인 top role 이면, 개인 top role도 작게 보여줌 */}
            {r.topRole && r.topRole !== "UNKNOWN" && r.role && r.role !== "UNKNOWN" && r.topRole !== r.role ? (
              <div className="p" style={{ marginTop: 4, fontSize: 12, opacity: 0.85 }}>
                note: personal top role = <b>{ROLE_KO[r.topRole] ? `${ROLE_KO[r.topRole]} (${r.topRole})` : r.topRole}</b>{" "}
                ({formatPct01(r.topShare)}; {Number.isFinite(Number(r.topGames)) ? r.topGames : "(n/a)"} /{" "}
                {Number.isFinite(Number(r.total)) ? r.total : "(n/a)"})
              </div>
            ) : null}
          </div>
